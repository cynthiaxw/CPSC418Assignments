...
public class Client 
{
    ...
    public void getKey() throws IOException {
    	...
    	BigInteger p;
    	BigInteger g;
    	BigInteger a = new BigInteger(CryptoUtilities.DH_BIT_LEN-1, new Random());
    	debug("Receiving p,g...");
    	byte[] p_byte = null;
    	byte[] g_byte = null;
    	int length = 0;
    	length = in.readInt();
    	p_byte = new byte[length];
    	in.read(p_byte);
    	
    	length = in.readInt();
    	g_byte = new byte[length];
    	in.read(g_byte);
    	
    	p = new BigInteger(p_byte);
    	g = new BigInteger(g_byte);
    	
    	debug("Received safe prime p = " + p);
    	debug("Received primitive root g = " + g);  
    debug("Secret a = " + a);
    
    byte[] ya_byte = null;
    byte[] yb_byte = null;

    debug("Receiving g^b (mod p)...");
    length = in.readInt();
    yb_byte = new byte[length];
    in.read(yb_byte);
    BigInteger yb = new BigInteger(yb_byte);
    debug("Received g^b (mod p) = "+ yb);
    
    BigInteger ya = g.modPow(a, p);
    //Send ya to Client
    ya_byte = ya.toByteArray();
    out.writeInt(ya_byte.length);
    	out.write(ya_byte);
    
    	BigInteger k = yb.modPow(a, p);
